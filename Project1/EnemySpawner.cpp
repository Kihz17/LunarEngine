...
{
	Entity* typeA = SpawnPhysicsSphere(glm::vec3(0.0f, 0.9f, 0.0f)); // Green
...
{
	Entity* typeB = SpawnPhysicsSphere(glm::vec3(0.0f, 0.0f, 0.6f)); // Blue
...
{
	Entity* typeC = SpawnPhysicsSphere(glm::vec3(0.6f, 0.0f, 0.0f)); // Red
	...
	typeCComp->AddTargetingBehaviour(0, new ApproachShootCondition(aiLayer->CreateBehaviour<SeekBehaviour>(typeC->GetComponent<RigidBodyComponent>()->ptr, 30.0f, SeekType::Approach, true, 100.0f), 
		5.0f, entityManager, physicsFactory, physicsWorld, enemyMesh));
}

void EnemySpawner::SpawnTypeD()
{
	Entity* typeD = SpawnPhysicsSphere(glm::vec3(0.6f, 0.6f, 0.0f)); // Yellow
...
Entity* EnemySpawner::SpawnPhysicsSphere(const glm::vec3& color)
{
	...
	sphereInfo.vao = enemyMesh->GetVertexArray();
	sphereInfo.indexCount = enemyMesh->GetIndexBuffer()->GetCount();
