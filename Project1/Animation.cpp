...
void Animation::ParseKeyFrames(const aiNode* assimpNode, const aiAnimation* assimpAnim, AnimatedMesh* mesh)
{
	std::string nodeName = assimpNode->mName.C_Str();

	aiNodeAnim* animationNode = nullptr;
	for (unsigned int i = 0; i < assimpAnim->mNumChannels; i++)
	{
		aiNodeAnim* assimpAnimNode = assimpAnim->mChannels[i];
		if (std::string(assimpAnimNode->mNodeName.C_Str()) == nodeName)
		{
			animationNode = assimpAnimNode;
			break;
		}
	}
	...
	if (animationNode)
	{
		if (!mesh->GetBoneInfo(nodeName)) // The mesh has this bone
		{
			std::cout << "WARNING: Bone was missing from rigged mesh for animation node '" << animationNode->mNodeName.C_Str() << "'!\n";
		}

		// Setup position key frames
		keyFrameMap[nodeName].positions.resize(animationNode->mNumPositionKeys);
		for (unsigned int i = 0; i < animationNode->mNumPositionKeys; i++)
		{
			aiVector3D assimpPos = animationNode->mPositionKeys[i].mValue;

			// Make new position Key Frame
			KeyFramePosition frame;
			frame.position = glm::vec3(assimpPos.x, assimpPos.y, assimpPos.z);
			frame.timeStamp = animationNode->mPositionKeys[i].mTime;
			keyFrameMap[nodeName].positions[i] = frame;
		}

		// Setup rotation key frames
		keyFrameMap[nodeName].rotations.resize(animationNode->mNumRotationKeys);
		for (unsigned int i = 0; i < animationNode->mNumRotationKeys; i++)
		{
			aiQuaternion assimpRot = animationNode->mRotationKeys[i].mValue;

			// Make new rotation Key Frame
			KeyFrameRotation frame;
			frame.rotation = glm::quat(assimpRot.w, assimpRot.x, assimpRot.y, assimpRot.z);
			frame.timeStamp = animationNode->mRotationKeys[i].mTime;
			keyFrameMap[nodeName].rotations[i] = frame;
		}

		// Setup scale key frames
		keyFrameMap[nodeName].scales.resize(animationNode->mNumScalingKeys);
		for (unsigned int i = 0; i < animationNode->mNumScalingKeys; i++)
		{
			aiVector3D assimpScale = animationNode->mScalingKeys[i].mValue;

			// Make new scale Key Frame
			KeyFrameScale frame;
			frame.scale = glm::vec3(assimpScale.x, assimpScale.y, assimpScale.z);
			frame.timeStamp = animationNode->mScalingKeys[i].mTime;
			keyFrameMap[nodeName].scales[i] = frame;
		}
	}

	for (unsigned int i = 0; i < assimpNode->mNumChildren; i++)
	{
		ParseKeyFrames(assimpNode->mChildren[i], assimpAnim, mesh);
	}
}